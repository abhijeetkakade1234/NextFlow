---
name: redteam-attack
description: Red-team any codebase, system design, API spec, architecture document, smart contract, or technical document to find exploits, attack vectors, and vulnerabilities — then produce prioritised fixes with solutions.
---

# Universal Red Team Attack & Solution Skill
Produces a structured offensive security review of ANY system, code, or architecture doc. Finds real exploits. Delivers prioritised fixes. Output = downloadable .docx report.

## Step 0 — Read the docx skill
Always read `/mnt/skills/public/docx/SKILL.md` before writing any generation code. Follow its rules exactly: dual table widths, `LevelFormat.BULLET`, `ShadingType.CLEAR`, DXA units.
...
2. **Who are the actors?** (users, admins, services, anonymous)
3. **Where does money / data / power flow?**
4. **What are the trust assumptions?** (each one is an attack surface)
5. **What are the time-dependent operations?** (race conditions live here)
6. **What external systems does it touch?** (each = supply chain risk)
...
- **CRITICAL** → Attacker can steal data/funds, take over accounts, or break core system integrity. Low effort required.
- **HIGH** → Significant damage possible. Attacker profit > attacker cost. Requires some effort but realistic.
- **MEDIUM** → Real vulnerability but limited blast radius, OR requires specific conditions / elevated access.
- **LOW** → Theoretical. Hard to exploit in practice, or impact is minor.
...
1. **Cover Page**: System name, "Red Team Report", date, CONFIDENTIAL
2. **Section 0: Scope & Methodology**: What was reviewed, attacker model, rating legend
3. **Section 1–N: Attacks by Category**:
    - One section per applicable attack layer
    - Each finding as: heading + attackEntry table (vector / impact / fix)
4. **Priority Matrix Table**: All findings ranked: ID | Title | Severity | Fix Effort | Sprint
5. **Immediate Fix Checklist**: Just the CRITICAL + HIGH findings as bullet checklist (Launch blockers)
